# -*- coding: utf-8 -*-
"""
Stage 1: Chapter Processing (Page-Level Records Version) - FIXED V2
...
def log_progress(message, end='\n'):
    ...
    FIXED: Now accepts 'end' parameter like print()
    """
    progress_logger = logging.getLogger('progress')
    
    # If end is not newline, we need to handle it specially
    if end == '':
        # Use a custom handler that doesn't add newline
        sys.stdout.write(message)
        sys.stdout.flush()
        # Still log to file with newline
...
def count_tokens(text: str) -> int:
    ...
    # FIXED: Clarified variable naming for token bounds
    MIN_CHARS_PER_TOKEN = 2     # Minimum characters per token (for dense text like code)
    MAX_CHARS_PER_TOKEN = 10    # Maximum characters per token (for sparse text)
    
    # Calculate bounds correctly
    max_possible_tokens = char_count // MIN_CHARS_PER_TOKEN  # Maximum estimate
    min_possible_tokens = char_count // MAX_CHARS_PER_TOKEN  # Minimum estimate
...
def call_gpt_chat_completion(client, model, messages, max_tokens, temperature, tools=None, tool_choice=None):
    ...
    for attempt in range(API_RETRY_ATTEMPTS):
        try:
            if attempt > 0:
                logging.info(f"API call retry attempt {attempt + 1}/{API_RETRY_ATTEMPTS}")
            ...
            logging.warning(f"Non-API Error on attempt {attempt + 1}: {e}")
            last_exception = e
            # Use exponential backoff consistently
            time.sleep(API_RETRY_DELAY * (2 ** attempt))
...
def parse_gpt_json_response(response_content_str: str, expected_keys: List[str] = None) -> Optional[Dict]:
    ...
    try:
        ...
        if expected_keys:
            ...
            if missing_keys:
                logging.warning(f"Missing expected keys in response: {', '.join(missing_keys)}")
                logging.debug(f"Available keys: {', '.join(data.keys())}")
        ...
        logging.debug(f"Raw response (first 500 chars): {response_content_str[:500]}...")
        return None
    except Exception as e:
...
def process_chapter_segment_for_details(segment_text, client, model_name, max_completion_tokens, temperature, prev_summary=None, prev_tags=None, is_final_segment=False):
    ...
    try:
        ...
        if 'summary' not in parsed_data:
            logging.warning("Response missing 'summary' field, using default")
            parsed_data['summary'] = "Summary generation failed"
        
        if 'tags' not in parsed_data:
            logging.warning("Response missing 'tags' field, using empty list")
            parsed_data['tags'] = []
...
def get_chapter_summary_and_tags(chapter_text: str, client: OpenAI, model_name: str = MODEL_NAME_CHAT) -> Tuple[Optional[str], Optional[List[str]]]:
    ...
    if needs_segmentation:
        ...
        for i, segment_text in enumerate(segments):
            ...
            # Retry logic for segment processing with logging
            segment_result = None
            for retry in range(3):
                try:
                    ...
                    if segment_result:
                        ...
                        time.sleep(5 * (2 ** retry))  # Exponential backoff
                except Exception as e:
        ...
        for retry in range(3):
            try:
                ...
                logging.warning(f"Exception in single call processing: {e}")
                if retry < 2:
                    time.sleep(5 * (2 ** retry))
...
def cleanup_logging_handlers():
    ...
    # Close and remove progress logger handlers
    progress_logger = logging.getLogger('progress')
...
def run_stage1():
    ...
    try:
        log_file_name = f"stage1_fixed_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
