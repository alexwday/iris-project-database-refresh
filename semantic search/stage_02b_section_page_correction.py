                ...
                if prev.get('section_end_page') is not None:
                    ...
                    if i + 1 < len(chapter_sections):
                        ...
                        if next_section.get('section_start_page') is not None:
                            ...
                            # No next section with page info - be conservative
                            # Check if there are any page tags after this section's position
                            # For now, assume it continues on the same page as previous
                            section['section_start_page'] = prev['section_end_page']
                            section['section_end_page'] = prev['section_end_page']
                            inferred = True
...
def run_stage2_5():
    ...
    if statistics.get('large_sections'):
        print("\n" + "-" * 70)
        print("⚠️  Large Sections Detected (>5 pages):")
        print(f"  Found {len(statistics['large_sections'])} sections spanning more than 5 pages")
        print(f"  Affected chapters: {statistics.get('chapters_with_large_sections', [])}")
        print("\n  Details (first 10):")
        for ls in statistics['large_sections'][:10]:
            print(f"    Chapter {ls['chapter']}, Section {ls['section']}: {ls['page_count']} pages ({ls['pages']})")
        if len(statistics['large_sections']) > 10:
            print(f"    ... and {len(statistics['large_sections']) - 10} more")
        print("\n  💡 Consider reviewing these sections for potential splitting")
    
    if SAMPLE_CHAPTER_LIMIT:
        print(f"\n  ⚠️  SAMPLE MODE: Limited to {SAMPLE_CHAPTER_LIMIT} chapters")
