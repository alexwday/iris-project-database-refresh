...
DO NOT provide a plain text response.
    ...
    user_prompt_parts.append(
        """Create a condensed summary in EXACTLY 2-3 sentences that captures:
...
def process_chapter_segment_robust(segment_text, client, model_name, prev_summary=None, is_final_segment=False):
    ...
    if result:
        ...
        if usage_info and VERBOSE_LOGGING:
            ...
            logging.debug(
                f"API Usage - Prompt: {prompt_tokens}, Completion: {completion_tokens}, Cost: ${total_cost:.4f}"
            )
...
def get_chapter_summary_robust(chapter_text: str, client: OpenAI, model_name: str = MODEL_NAME_CHAT) -> Optional[str]:
    ...
    if not chapter_text or not chapter_text.strip():
        logging.warning("Chapter text is empty or contains only whitespace. Returning None.")
        return None
    ...
    if total_tokens == 0:
        logging.warning(f"Chapter text has {total_chars} characters but 0 tokens. Returning None.")
        return None
    ...
    if needs_segmentation:
        ...
        if available_for_content <= 0:
            logging.error(
                f"Invalid available_for_content: {available_for_content}. Cannot segment chapter with no available token space."
            )
            return None
        ...
        if token_difference > token_tolerance:
            log_progress(
                f"    ⚠️ Token count mismatch: original {total_tokens:,} vs sum {total_segment_tokens:,} (diff: {token_difference:,})"
            )
        ...
        for seg_idx, (original_idx, segment_text) in enumerate(non_empty_segments):
            ...
            log_progress(
                f"  🔄 Processing segment {seg_idx+1}/{len(non_empty_segments)} ({segment_tokens:,} tokens)...", end=""
            )
            ...
            for retry in range(3):
                try:
                    segment_result = process_chapter_segment_robust(
                        segment_text, client, model_name, prev_summary=current_summary, is_final_segment=is_final
                    )
                    if segment_result:
                        break
                    elif retry < 2:
        ...
        log_progress(f"  📄 Chapter size: {total_chars:,} chars ({total_tokens:,} tokens)")
        log_progress("  🔄 Processing in single call...", end="")

        result = None
        for retry in range(3):
            try:
                result = process_chapter_segment_robust(chapter_text, client, model_name, is_final_segment=True)
                if result:
                    break
                elif retry < 2:
...
def process_chapter_pages(chapter_num: int, pages: List[Dict], client: Optional[OpenAI]) -> List[Dict]:
    ...
    if client:
        try:
            chapter_summary = get_chapter_summary_robust(concatenated_content, client, model_name=MODEL_NAME_CHAT)
            if chapter_summary:
                log_progress("  ✅ Generated summary")
            else:
                log_progress("  ⚠️ Failed to generate summary")
        except Exception as e:
...
def run_stage1():
    ...
    try:
        ...
        if write_to_nas(share_name, output_path_relative, output_bytes):
            log_progress("✅ Successfully saved output")
        else:
