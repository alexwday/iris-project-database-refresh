# -*- coding: utf-8 -*-
"""
Stage 2: Section Identification & Enrichment (NAS-Integrated Version)

Purpose:
Processes chapter data generated by Stage 1 NAS script. It identifies logical sections
...
Input: JSON file from Stage 1 NAS output on the NAS drive.
       (e.g., 'semantic_search/pipeline_output/stage1/stage1_chapter_data.json')
Output: A JSON file on the NAS containing a list of section dictionaries.
        (e.g., 'semantic_search/pipeline_output/stage2/stage2_section_data.json')
"""
...
# Path on NAS where stage1 output is stored (relative to share root)
NAS_STAGE1_OUTPUT_PATH = "semantic_search/pipeline_output/stage1"
STAGE1_FILENAME = "stage1_chapter_data.json"
# Path on NAS where stage2 output will be saved (relative to share root)
...
def get_openai_client(base_url=BASE_URL) -> Optional[OpenAI]:
    ...
    if not _setup_ssl_from_nas(): logging.warning("Proceeding without explicit SSL setup.")
...
def run_stage2():
    """Main function to execute Stage 2 processing with NAS integration and resumability."""
    # Setup logging
    temp_log_path = setup_logging()
    
    logging.info("--- Starting Stage 2: Section Identification & Enrichment (NAS-Integrated) ---")
    ...
    if existing_json_bytes:
        try:
            existing_data_list = json.loads(existing_json_bytes.decode('utf-8'))
            if isinstance(existing_data_list, list):
                ...
                for sec_data in existing_data_list:
                    ...
                    if sec_id:
                        ...
                        count += 1
                logging.info(f"Loaded {count} existing section records from NAS.")
            else:
    ...
    # --- Load Stage 1 Data from NAS ---
    stage1_path_relative = os.path.join(NAS_STAGE1_OUTPUT_PATH, STAGE1_FILENAME).replace('\\', '/')
    logging.info(f"Loading Stage 1 data from NAS: {share_name}/{stage1_path_relative}")
    
    stage1_json_bytes = read_from_nas(share_name, stage1_path_relative)
    if stage1_json_bytes is None:
        logging.error(f"Stage 1 output file not found on NAS: {stage1_path_relative}. Cannot proceed.")
        sys.exit(f"Error: Stage 1 output file not found on NAS.")
    
    try:
        all_chapter_data = json.loads(stage1_json_bytes.decode('utf-8'))
        logging.info(f"Loaded {len(all_chapter_data)} chapters from Stage 1")
    except Exception as e:
        logging.error(f"Error loading Stage 1 data from NAS: {e}", exc_info=True)
        sys.exit(f"Error: Failed to load Stage 1 data from NAS.")

    if not all_chapter_data:
        logging.warning("Stage 1 data is empty. No sections to process.")
        # Save the potentially loaded (but empty) existing data back to NAS
        if existing_section_details:
            json_bytes = json.dumps(list(existing_section_details.values()), indent=2, ensure_ascii=False).encode('utf-8')
            write_to_nas(share_name, output_path_relative, json_bytes)
            logging.info(f"No chapters to process. Saved {len(existing_section_details)} existing section records back to NAS.")
    ...
    for chapter_data in tqdm(all_chapter_data, desc="Processing Chapters for Sections"):
        ...
        # --- Incremental Save to NAS (after each chapter) ---
        try:
            ...
            # Write the sorted list to NAS, overwriting previous content
            ...
            if write_to_nas(share_name, output_path_relative, json_bytes):
                logging.info(f"Incrementally saved {len(temp_sorted_data)} total section records to NAS after processing Chapter {chapter_number}")
            else:
                logging.error(f"Failed to save incremental update to NAS after processing Chapter {chapter_number}")

        except Exception as e:
            logging.error(f"Error during incremental save to NAS after processing Chapter {chapter_number}: {e}", exc_info=True)
            # Continue processing other chapters, but progress might be lost if script fails later

    # --- Upload Log File to NAS ---
    try:
        log_file_name = f"stage2_section_processing_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    ...
    print(f"Total chapters from Stage 1: {len(all_chapter_data)}")
    print(f"Total sections in final file : {final_record_count}")
