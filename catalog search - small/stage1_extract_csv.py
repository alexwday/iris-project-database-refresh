...
if __name__ == "__main__":
    ...
    print("[1] Initializing Master CSV Files on NAS...")
    ...
    if not write_json_to_nas(NAS_PARAMS["share"], csv_output_relative_file, csv_json_string):
         print("   [CRITICAL ERROR] Failed to write CSV catalog JSON to NAS. Exiting.")
         sys.exit(1)
    print("-" * 60)
    ...
    if FULL_REFRESH:
        ...
        if csv_df.empty and nas_df.empty:
            ...
            if 'file_name' not in csv_df.columns:
                print("   [CRITICAL ERROR] 'file_name' column missing in CSV DataFrame. Exiting.")
                sys.exit(1)
